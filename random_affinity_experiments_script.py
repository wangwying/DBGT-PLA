...
if __name__ == "__main__":
    ...
    # randomly shuffle affinities and assign them to the interactions
    if RANDOMIZER == "shuffle":
        rng = np.random.default_rng(seed = SEED)
        rng.shuffle(vals_cleaned)
        affinities_df['affinity'] = vals_cleaned

        print("First 10 interactions with shuffle affinities:")

    elif RANDOMIZER == "random":
        rng = np.random.default_rng(seed = SEED)
        vals_random = rng.uniform(low = MIN_AFFINITY, high = MAX_AFFINITY, size = len(vals_cleaned))
        affinities_df['affinity'] = vals_random

        print("First 10 interactions with randomly assigned affinities:")
    else:
        print("Wrong randomizer name. Choose between shuffle and random.")
        sys.exit()

    print(affinities_df.head(10))
        ...
        for file in tqdm(dirs):
            ...
            if interaction_name in interaction_affinities:
                if os.path.isdir(data_path + interaction_name):
                    ...
                    G = None
                    with open(data_path + interaction_name + "/" + interaction_name + "_interaction_graph.json", 'r',
                              encoding='utf-8') as f:
                        data = json.load(f)
                        G = nx.Graph()
                    ...
                    if not NODE_FEATURES:
                        dataset_dict[interaction_name]["x"] = torch.full((num_nodes, 1), 1.0,dtype=torch.float)
                    else:
    ...
    print("Numbr of test medium affinity samples: ", len([sample for sample in core_set_data if sample.y >= MEAN_LOWER_BOUND and sample.y <= MEAN_UPPER_BOUND]))
    ...
    def pearson_loss(y_pred, y_true):
        ...
        loss = 1 - torch.sum(x * y) / (torch.sqrt(torch.sum(x ** 2)) * torch.sqrt(torch.sum(y ** 2)))
        return loss
    ...
    class CombinedLoss(nn.Module):
        ...
        def forward(self, y_pred, y_true):
            ...
            pcc = pearson_loss(y_pred, y_true)
            loss = (self.alpha * huber
                    + self.beta * pcc)
    ...
    for epoch in range(epochs):
        ...
        print(
            f'Epoch: {epoch:03d}, Train RMSE: {train_rmse:.4f}, Val RMSE: {val_rmse:.4f}, Train MAE: {train_mae:.4f}, Val MAE: {val_mae:.4f}, Train SD: {train_SD:.4f}, Val SD: {val_SD:.4f}')
    if early_stop:
    ...
    if not SAVE_BEST_MODEL:
        print(
            f'Core set 2013 RMSE with latest model: {core_set_2013_rmse:.4f}, MSE: {core_set_2013_mae:.4f}, SD: {core_set_2013_SD:.4f}')
        print(
            f'Core set 2016 RMSE with latest model: {core_set_2016_rmse:.4f}, MSE: {core_set_2016_mae:.4f}, SD: {core_set_2016_SD:.4f}')
        print(
            f'Hold out set 2019 RMSE with latest model: {hold_out_set_rmse:.4f}, MSE: {hold_out_set_mae:.4f}, SD: {hold_out_set_SD:.4f}')
